...
# sensors_data <- sensor_ids %>% filter(sensor_ids$detector %in% config_data$detector_name == TRUE)

sensor_matrix <- data.matrix(sensor_ids)
...
foreach(j = sensor_matrix[737:738]) %dopar% {
